import { an as assign, k as assertNever, d as opcodes, j as dict } from './main-CjWhnG7r.js';
...
class J {
  ...
  static load(e, s) {
    return "number" == typeof s ? J.forCharPositions(e, s, s) : "string" == typeof s ? J.synthetic(s) : Array.isArray(s) ? J.forCharPositions(e, s[0], s[1]) : s === $ ? J.NON_EXISTENT : s === z ? J.broken(B) : void assertNever(s);
  }
...
var Mt = {
  ...
  block: function (t, e, s, r, n = null, a, i, o, l) {
    ...
    return c = "Template" === r.type ? Vt.blockItself({
      ...
      body: r.body,
      loc: r.loc
    }) : r, "Template" === n?.type ? (n.blockParams.length, h = Vt.blockItself({
      params: [],
      body: n.body,
      loc: n.loc
    })) : h = n, Vt.block({
  ...
  element: function (t, e = {}) {
    ...
    "string" == typeof t ? t.endsWith("/") ? (s = Lt(t.slice(0, -1)), r = true) : s = Lt(t) : "type" in t ? (t.type, t.type, s = t) : "path" in t ? (t.path.type, t.path.type, s = t.path, r = t.selfClosing) : (s = Lt(t.name), r = t.selfClosing);
...
class jt extends Ut {
  ...
  BlockStatement(t) {
    if ("comment" === this.tokenizer.state) return t.loc, void this.appendToCommentData(this.sourceForNode(t));
...
class Gt extends jt {
  ...
  parsePossibleBlockParams() {
    ...
    const n = {
      ...
      BeforeStartPipe: t => {
        r.state, Wt(t) ? this.tokenizer.consume() : "|" === t ? (r = {
      ...
      BeforeBlockParamName: t => {
        if (r.state, Wt(t)) this.tokenizer.consume();else if ("" === t) r = {
          state: "Done"
        }, this.pendingError = {
          mustache(t) {
      ...
      Error: t => {
        if (r.state, "" === t || "/" === t || ">" === t || Wt(t)) throw ot(r.message, r.start.until(this.offset()));
        this.tokenizer.consume();
      },
      Done: () => {}
    };
...
function Qt(t, s = {}) {
  ...
  if (s.plugins?.ast) for (const t of s.plugins.ast) St(h, t(assign({}, s, {
    syntax: Yt
  }, {
    plugins: void 0
  })).visitor);
...
var Ie = Object.freeze({
  ...
  LooseModeResolution: Fe,
  MODIFIER_NAMESPACE: ze,
  NamedArgument: ne,
  NamedArguments: re,
  NamedBlock: Pe,
  NamedBlocks: xe,
  ...
  loadResolution: function (t) {
    return "Strict" === t ? Oe : "ComponentOrHelper" === t ? Fe.append() : Fe.namespaced(t);
  },
  node: te
});
class _e {
...
class Ve extends _e {
  constructor(t, e, r) {
    super(), this.templateLocals = t, this.keywords = e, this.options = r, this.symbols = [], this.upvars = [], this.size = 1, this.named = dict(), this.blocks = dict(), this.usedTemplateLocals = [];
  }
  root() {
    return this;
  }
  ...
  allocateFree(t, e) {
    e.resolution() === opcodes.GetFreeAsComponentHead && e.isAngleBracket && (t = this.options.customizeComponentName(t));
    let s = this.upvars.indexOf(t);
    return -1 !== s || (s = this.upvars.length, this.upvars.push(t)), s;
  }
  allocateNamed(t) {
    let e = this.named[t];
...
class je {
  constructor(t) {
    this.base = t, this.builder = new De();
  }
  simple(t, s, r) {
    return new fe(assign({
      tag: t,
      body: s,
      componentArgs: [],
      loc: r
  ...
  componentWithNamedBlocks(t, s, r) {
    return new me(assign({
      loc: r,
      callee: t,
      blocks: this.builder.namedBlocks(s, st.range(s))
    }, this.base));
  }
}
function Je(t) {
  return qe(t) ? Fe.namespaced($e) : null;
}
function Ke(t) {
  return qe(t) ? Fe.namespaced(ze) : null;
}
function Re(t) {
  return qe(t) ? Fe.namespaced(Me) : null;
}
function We(t) {
  return Qe(t) ? Fe.namespaced(Me, true) : null;
}
function Ge(t) {
  return qe(t) ? Fe.namespaced($e) : null;
}
function Ye(t) {
  let e = qe(t),
    s = t.trusting;
  return e ? s ? Fe.trustingAppend() : Fe.append() : null;
}
function qe(t) {
  return Qe(t.path);
}
...
class Ze {
  constructor(t, e, s) {
    this.source = t, this.options = e, this.table = s, this.builder = new De();
  }
  get strict() {
...
class ts {
      ...
      case "SubExpression":
        {
          ...
          let e = this.block.resolutionFor(t, Je);
          if ("error" === e.result) throw ot(`You attempted to invoke a path (\`${e.path}\`) but ${e.head} was not in scope`, t.loc);
  ...
  callParts(t, e) {
    let {
      ...
      l = st.range(o, i.loc.collapse("end")),
    ...
    switch (i.type) {
      case "Literal":
        throw ot(`Invalid invocation of a literal value (\`${i.value}\`)`, a);
      case "Interpolate":
        throw ot("Invalid invocation of a interpolated string", a);
    }
    return {
      callee: i,
...
class es {
  ...
  MustacheStatement(t) {
    ...
    if (os(s)) 0 === r.length && 0 === n.pairs.length ? e = this.expr.normalize(s) : ls(s, i);else {
      let a = this.block.resolutionFor(t, Ye);
      if ("error" === a.result) throw ot(`You attempted to render a path (\`{{${a.path}}}\`), but ${a.head} was not in scope`, i);
  ...
  BlockStatement(t) {
    ...
    let a = this.block.resolutionFor(t, Re);
    if ("error" === a.result) throw ot(`You attempted to invoke a path (\`{{#${a.path}}}\`) but ${a.head} was not in scope`, n);
...
class ss {
  ...
  modifier(t) {
    os(t.path) && ls(t.path, t.loc);
    let e = this.ctx.resolutionFor(t, Ke);
    if ("error" === e.result) throw ot(`You attempted to invoke a path (\`{{${e.path}}}\`) as a modifier, but ${e.head} was not in scope`, t.loc);
  ...
  mustacheAttr(t) {
    ...
    let a = this.ctx.resolutionFor(t, Ge);
    if ("error" === a.result) throw ot(`You attempted to render a path (\`{{${a.path}}}\`), but ${a.head} was not in scope`, t.loc);
  ...
  attr(t) {
    if (t.name[0], "...attributes" === t.name) return this.ctx.builder.splatAttr(this.ctx.table.allocateBlock("attrs"), this.ctx.loc(t.loc));
  ...
  classifyTag(t, e, s) {
    ...
    if (this.ctx.strict && !a) {
      if (r) throw ot(`Attempted to invoke a component that was not in scope in a strict mode template, \`<${t}>\`. If you wanted to create an element with that name, convert it to lowercase - \`<${t.toLowerCase()}>\``, s);
      return "ElementHead";
    }
    let i = a || r,
      o = s.sliceStartChars({
    ...
    if (i) {
      let r = Vt.path({
        ...
        n = this.ctx.isLexicalVar(t) ? {
          result: Oe
        } : this.ctx.resolutionFor(r, We);
      if ("error" === n.result) throw ot(`You attempted to invoke a path (\`<${n.path}>\`) but ${n.head} was not in scope`, s);
      return new ts(this.ctx).normalize(r, n.result);
    }
    if (this.ctx.table.allocateFree(t, Oe), e.length > 0) throw ot(`You used ${t}.${e.join(".")} as a tag name, but ${t} is not in scope`, s);
    return "ElementHead";
  }
  get expr() {
...
class ns extends rs {
  assertTemplate(t) {
    if (E(this.namedBlocks)) throw ot("Unexpected named block at the top-level of a template", this.loc);
    return this.block.builder.template(t, this.nonBlockChildren, this.block.loc(this.loc));
  }
}
class as extends rs {
  assertBlock(t) {
    if (E(this.namedBlocks)) throw ot("Unexpected named block nested in a normal block", this.loc);
...
class is extends rs {
  ...
  assertNamedBlock(t, e) {
    if (this.el.base.selfClosing) throw ot(`<:${t.chars}/> is not a valid named block: named blocks cannot be self-closing`, this.loc);
    if (E(this.namedBlocks)) throw ot(`Unexpected named block inside <:${t.chars}> named block: named blocks cannot contain nested named blocks`, this.loc);
    if ((s = t.chars)[0] !== s[0]?.toLowerCase() || s[0] === s[0]?.toUpperCase()) throw ot(`<:${t.chars}> is not a valid named block, and named blocks must begin with a lowercase letter`, this.loc);
    var s;
    if (this.el.base.attrs.length > 0 || this.el.base.componentArgs.length > 0 || this.el.base.modifiers.length > 0) throw ot(`named block <:${t.chars}> cannot have attributes, arguments, or modifiers`, this.loc);
    let r = st.range(this.nonBlockChildren, this.loc);
    return this.block.builder.namedBlock(t, this.block.builder.block(e, this.nonBlockChildren, r), this.loc);
  }
  assertElement(t, e) {
    if (e) throw ot(`Unexpected block params in <${t.chars}>: simple elements cannot have block params`, this.loc);
    if (E(this.namedBlocks)) {
      let e = this.namedBlocks.map(t => t.name);
      if (1 === e.length) throw ot(`Unexpected named block <:foo> inside <${t.chars}> HTML element`, this.loc);
      {
        let s = e.map(t => `<:${t.chars}>`).join(", ");
        throw ot(`Unexpected named blocks inside <${t.chars}> HTML element (${s})`, this.loc);
      }
    }
    return this.el.simple(t, this.nonBlockChildren, this.loc);
  }
  assertComponent(t, e, s) {
    if (E(this.namedBlocks) && this.hasSemanticContent) throw ot(`Unexpected content inside <${t}> component invocation: when using named blocks, the tag cannot contain other content`, this.loc);
    if (E(this.namedBlocks)) {
      if (s) throw ot(`Unexpected block params list on <${t}> component invocation: when passing named blocks, the invocation tag cannot take block params`, this.loc);
      let e = new Set();
      for (let t of this.namedBlocks) {
        let s = t.name.chars;
        if (e.has(s)) throw ot(`Component had two named blocks with the same name, \`<:${s}>\`. Only one block with a given name may be passed`, this.loc);
        if ("inverse" === s && e.has("else") || "else" === s && e.has("inverse")) throw ot("Component has both <:else> and <:inverse> block. <:inverse> is an alias for <:else>", this.loc);
        e.add(s);
      }
...
function ls(t, e) {
  throw ot(`Unexpected literal \`${"StringLiteral" === t.type ? JSON.stringify(t.value) : String(t.value)}\``, e);
}
function cs(t) {
...
const _importSync40 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
    ...
    builders: Mt,
    cannotRemoveNode: ht,
    cannotReplaceNode: ut,
    generateSyntaxError: ot,
    getTemplateLocals: Zt,
    getVoidTags: k,
    hasSpan: nt,
    isKeyword: x,
    isVoidTag: y,
...
export { Ie as I, P, Xe as X, Zt as Z, _importSync40 as _, at as a, it as i, j, ot as o, rt as r, te as t, x };
